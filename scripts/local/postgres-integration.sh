...
readonly test_role_prefix="whetstone_test_"
...
test_suffix="$(uuidgen | tr -d '-' | tr 'A-F' 'a-f')"
test_role="${test_role_prefix}${test_suffix}"
test_database="${test_database_prefix}${test_suffix}"
readonly test_role test_database

if [[ ! "${test_role}" =~ ^whetstone_test_[0-9a-f]{32}$ ]]; then
  echo "generated test role name is invalid" >&2
  exit 1
fi
...
if [[ -n "${existing_role}" ]]; then
  echo "refusing to overwrite existing role ${test_role}" >&2
  exit 1
fi
...
cleanup() {
  ...
  local cleanup_status="${run_status}"
  ...
  if [[ ! "${test_role}" =~ ^whetstone_test_[0-9a-f]{32}$ ]]; then
    echo "refusing to drop an unexpectedly named role" >&2
    return 1
  fi
...
SQL
  ...
  if [[ "${role_created}" == true ]]; then
    if ! "${admin_psql[@]}" --set=test_role="${test_role}" \
      >/dev/null <<'SQL'
DROP ROLE :"test_role";
SQL
    then
      echo "failed to drop ${test_role}; it remains for inspection" >&2
      cleanup_status=1
    else
      role_created=false
      echo "Removed disposable PostgreSQL role ${test_role}"
    fi
  fi
