...
def test_official_artifacts_require_full_typed_references() -> None:
    reference = TypedRef(
        schema_name="whetstone.evaluation_evidence",
        content_hash="a" * 64,
    )
    artifacts = CellArtifacts(
        official_record_before=reference,
        official_record_after=reference,
    )

    assert artifacts.model_dump(mode="json")["official_record_before"] == {
        "schema_name": "whetstone.evaluation_evidence",
        "content_hash": "a" * 64,
    }
    with pytest.raises(ValidationError):
        CellArtifacts(official_record_before="a" * 64)
...
def test_null_delta_and_ci_allowed() -> None:
    record = _record(
        delta=None, ci95=None, baseline_official=None, best_official=None
    )
...
def test_attempt_lookup_never_returns_another_attempt(tmp_path: Path) -> None:
    ledger = Ledger(root=tmp_path)
    attempt_zero = _record(attempt=0, cell_id="copro:c11:a0")
    attempt_one = _record(attempt=1, cell_id="copro:c11:a1")
    ledger.append_cell(attempt_zero)
    ledger.append_cell(attempt_one)

    assert ledger.for_attempt("copro", "c11", 0) == attempt_zero
    assert ledger.for_attempt("copro", "c11", 1) == attempt_one
    assert ledger.for_attempt("copro", "c11", 2) is None
