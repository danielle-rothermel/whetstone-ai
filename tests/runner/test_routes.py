...
def test_route_for_openai_lane_dispatch() -> None:
    ...
    prop = route_for("openai", role="proposer", proposer_model="gpt-5.4-nano")
...
def test_temperature_folds_into_config_identity() -> None:
    ...
    assert t0.call_config.identity_hash != t1.call_config.identity_hash
...
def test_reasoning_effort_folds_into_config_identity_openrouter() -> None:
    ...
    hashes = {
        base.call_config.identity_hash,
        low.call_config.identity_hash,
        none.call_config.identity_hash,
        high.call_config.identity_hash,
    }
...
def test_temperature_zero_vs_none_distinct_and_on_wire() -> None:
    ...
    t0 = route_for(
        "openrouter", role="task", task_model="gpt-5-nano", temperature=0.0
    )
    t5 = route_for(
        "openrouter", role="task", task_model="gpt-5-nano", temperature=0.5
    )
    tn = route_for(
        "openrouter", role="task", task_model="gpt-5-nano", temperature=None
    )
    ids = {
        t0.call_config.identity_hash,
        t5.call_config.identity_hash,
