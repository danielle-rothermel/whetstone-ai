...
def content_ref(schema: str, char: str) -> TypedContentRef:
    return TypedContentRef(schema_name=schema, content_hash=full_hash(char))


def record_ref(char: str) -> TypedContentRef:
    return content_ref("whetstone.materialization_record", char)


def aggregate_ref(char: str) -> TypedContentRef:
    return content_ref("whetstone.rollout_aggregate", char)


def result_ref(char: str) -> TypedContentRef:
    return content_ref("whetstone.rollout_result", char)


def oer_ref(char: str) -> TypedContentRef:
