...
def test_loader_reads_mutants_directly() -> None:
    ...
    assert all(
        isinstance(e, ExpectedOutcome) for e in m.mutant_expected
    )
...
def test_oracle_infrastructure_unknown_when_runner_always_fails() -> None:
    ...
    s = score_ed1m_reconstruction(
        reconstruction=m.mutated_full_source, mutant=m,
        run_in_subprocess=_boom, timeout_seconds=5.0,
    )
...
def test_build_ed1m_experiment_no_budget_default() -> None:
    ...
    exp = build_ed1m_experiment(
        mutants=mutants, internal_n=3, official_n=3,
...
def test_ed1m_eval_rewards_fidelity_reports_attractor() -> None:
    ...
    exp = build_ed1m_experiment(
        mutants=mutants, internal_n=2, official_n=2,
        blend_config=BoundedCompressionMetricConfig(weight=0.1),
    )
    ...
    ed = run_ed1_eval(
        ...
        transport=FakeTransport(reply=reply), repeats=1, apply_reward=True,
    )
...
@_skip_no_artifact
def test_ed1m_optimizer_cell_refuses_pass_only() -> None:
    # (Task 22.3 guard rail extends to ed1m) a searching ed1m optimizer cell
    # with no blend config is refused.
    from tests.envs.support import FakeTransport, execution_policy
    ...
    cfg = CellConfig(
        optimizer="miprov2", env="ed1m", lane="openrouter", attempt=0,
        task_model="deepseek/deepseek-v4-flash", proposer_model="none",
        canonical=True, proposer_config=proposer_config(),
        ...
        budget_ratio=None, ed1_task_limit=2, ed1_blend_config=None,
    )
...
def _synthetic_mutant() -> Ed1mMutant:
    ...
    return Ed1mMutant(
        ...
        input_reprs=("[1]", "[5]"),
        mutant_expected=(
            ExpectedOutcome("value", "0"), ExpectedOutcome("value", "4"),
        ),
        canonical_expected=(
            ExpectedOutcome("value", "2"), ExpectedOutcome("value", "6"),
        ),
