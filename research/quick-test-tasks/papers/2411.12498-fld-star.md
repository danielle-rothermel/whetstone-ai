...
affiliations: "Advanced AI Innovation Center, Hitachi (Morishita, Morio, Sogawa); The University of Sheffield (Yamaguchi)"
venue: "NeurIPS 2024 (per arXiv Comments field). v1 submitted 2024-11-19; v2 submitted 2024-12-23 (LaTeXML-rendered 2024-12-23, used for this extraction)."
fetch_route: "Step 2 of acquisition chain — curl -sL https://arxiv.org/html/2411.12498 (resolved to v2, LaTeXML-generated HTML, ~1.4MB), parsed locally with python3 + BeautifulSoup (html.parser) to walk the DOM and extract headings, paragraphs, figure/table captions, table cell contents, footnotes, and the full bibliography, preserving section structure and reading order."
...
- **Title/short-name mismatch, same paper.** The task brief refers to this paper as "FLD*" (a follow-up to FLD with proved/disproved/unknown labels under controllable depth and distractors). The actual arXiv title at 2411.12498 is **"Enhancing Reasoning Capabilities of LLMs via Principled Synthetic Logic Corpus"** (Morishita, Morio, Yamaguchi, Sogawa). The paper does NOT use the name "FLD*" anywhere in its text; instead it introduces a synthetic corpus called **"Formal Logic Deduction Diverse" (FLD×2, stylized "FLD²")**, built on top of the earlier FLD corpus (Morishita et al., 2023), and a training method called "Additional Logic Training (ALT)." This is clearly the paper the task intends (Morishita et al., NeurIPS 2024, arXiv 2411.12498, a follow-up to FLD with proved/disproved/unknown labels, controllable depth (1-8 logical steps), and adversarial/controllable distractors) — the arXiv ID resolves correctly and the content matches the candidate description (FLD follow-up, depth control via "s" logical steps, distractors via Design Principle 2, proved/disproved/unknown-style answer labels described in Appendix D.1). The "FLD*" label appears to be a project-internal shorthand for FLD×2, not the paper's own title.
- Authors: WebFetch's AI summary and the raw HTML author byline agree: Terufumi Morishita, Gaku Morio, Atsuki Yamaguchi, Yasuhiro Sogawa. No discrepancy.
- Venue: arXiv Comments field / WebFetch summary states "NeurIPS 2024." No independent journal-ref field was inspected beyond the WebFetch summary render; this is consistent with the paper's own NeurIPS-style checklist appendix ("NeurIPS Paper Checklist") present in the full text below.
- Versions: v1 (2024-11-19), v2 (2024-12-23). The HTML fetched via `https://arxiv.org/html/2411.12498` redirected to v2 (confirmed via `<base href="/html/2411.12498v2/">` and the LaTeXML generation timestamp "Mon Dec 23 10:37:26 2024" in the HTML source).

# Extraction notes / known issues

- **Route used:** Step 2 of the acquisition chain succeeded on the first try — `curl -sL https://arxiv.org/html/2411.12498` returned a full LaTeXML-generated HTML document (~1.4 MB) with real body text, so no fallback routes (ar5iv, e-print/LaTeX source, or PDF) were needed.
- **Parsing:** A custom Python (BeautifulSoup `html.parser`) DOM walker was written to traverse the `<article>` element, preserving heading hierarchy (`#`/`##`/`###` mapped from LaTeXML's `ltx_title_*` classes), paragraph text, figure captions, table captions, table cell contents (row-by-row, cells joined with ` | `), footnotes, and the full bibliography (`ltx_bibitem` list, 135 entries). All of Sections 1-7, Acknowledgements, References, Appendices A-F, and the NeurIPS Paper Checklist were recovered in reading order.
- **Math/equation rendering artifact:** LaTeXML renders inline/display math with duplicated content (a Unicode/plaintext rendering immediately followed by a MathML/LaTeX-source rendering, e.g. "ℱ ℱ \mathcal{F} caligraphic_F" or "43.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}}..."). This produces visibly redundant tokens inside formulas, proof-tree tables (`\AxiomC`, `\BinaryInfC`, `\DisplayProof` macro names leak through as literal text since LaTeXML could not fully render the `bussproofs`-style proof trees to HTML), and numeric table cells. This is a faithful (if visually noisy) transcription of what LaTeXML actually produced — it was not cleaned/normalized further, per the hard verbatim rule, since attempting to "fix" it would risk silently altering the authors' actual numeric/symbolic content.
- **Figures:** All figures (Figure 1, Figure 2, Figure D.3, etc.) are marked "[FIGURE: image not rendered]" with their full verbatim caption text preserved immediately below the marker. No image content itself could be transcribed (this is a text-only extraction).
- **Tables:** All tables (Tables 1, 2, 3, D.5-D.6, E.7, F.8, etc.) were extracted with row/cell text content preserved; captions are marked as "Caption: ...". Some nested/complex tables (e.g., Table 1's multi-row corpus-comparison cells) show minor structural duplication because LaTeXML nests `<table>` elements inside individual cells for multi-line entries — the duplication is a structural artifact of the walker re-emitting nested table rows, not fabricated or paraphrased text; every distinct piece of text visible in the duplication is verbatim from the source.
- **Self-check performed:** Three sentences were independently verified as exact (whitespace-normalized) substrings of the raw LaTeXML HTML's plain-text rendering (via BeautifulSoup `.get_text()`): (1) "LLMs can solve famous arithmetic problems as is but not when the numbers or names are changed" (Introduction), (2) "We observed a substantial improvement of up to 30 points on logical reasoning tasks" (Introduction), (3) "RuleTaker Clark et al. ( 2021 ) proposed a deduction corpus composed of synthetically generated multistep deductive proofs written in natural languages." (Appendix A.2, Related Work). All three matched exactly, confirming the body text below is verbatim (not paraphrased/reconstructed).
- WebFetch on the abs page was used ONLY for metadata (title/authors/abstract-summary/venue/version confirmation), per the acquisition-chain instructions; none of its (AI-summarized) text was used as body content below — all body text below comes from the raw HTML parse.

---

# Full Text (verbatim, extracted from arXiv HTML v2, LaTeXML rendering)

# Enhancing Reasoning Capabilities of LLMs via Principled Synthetic Logic Corpus

## Abstract

Large language models (LLMs) are capable of solving a wide range of tasks, yet they have struggled with reasoning. To address this, we propose Additional Logic Training (ALT) , which aims to enhance LLMs’ reasoning capabilities by program-generated logical reasoning samples. We first establish principles for designing high-quality samples by integrating symbolic logic theory and previous empirical insights. Then, based on these principles, we construct a synthetic corpus named Formal Logic D eduction D iverse (FLD ×2 ), comprising numerous samples of multi-step deduction with unknown facts, diverse reasoning rules, diverse linguistic expressions, and challenging distractors. Finally, we empirically show that ALT on FLD ×2 substantially enhances the reasoning capabilities of state-of-the-art LLMs, including LLaMA-3.1-70B. Improvements include gains of up to 30 points on logical reasoning benchmarks, up to 10 points on math and coding benchmarks, and 5 points on the benchmark suite BBH.
...
[FIGURE: image not rendered]
Caption: Figure 1 : The performance gains to LLaMA-3.1-70B by Additional Logic Training (ALT) on the proposed synthetic corpus, FLD ×2 (Formal Logic D eduction D iverse). Each benchmark set, such as “Logic” and “Math”, comprises various benchmarks in that domain. Tables 2 , 4 shows the details.
...
We then empirically verify that ALT can enhance LLMs’ reasoning capabilities ( Sections 4 , 5 ). Using 31 benchmarks covering diverse tasks, we observed that ALT on FLD ×2 substantially boosts state-of-the-art LLMs’ reasoning capabilities. Even LLaMA-3.1-70B, the largest LLM pre-trained on over 15 trillion tokens, shows substantial improvements with ALT ( Figure 1 ). Among synthetic logic corpora with different sample designs, FLD ×2 yielded the largest performance gains, validating our proposed design principles. Moreover, we discovered that employing a knowledge-forgetting prevention method during ALT is critically important, as it likely prevents the LLM’s knowledge of established facts from being displaced by the unknown facts included in synthetic logic corpora.

Finally, we analyze which task-solving capabilities ALT can enhance and why ( Section 6 ). We observed a substantial improvement of up to 30 points on logical reasoning tasks ( Figure 2(c) ). Surprisingly, we also observed improvements in abductive reasoning tasks, which go beyond the synthetic logic corpora’s original deductive reasoning tasks. Case analyses indicate that these improvements result from LLMs having acquired the fundamentals of the logic reflected in the design principles. We also observed improvements of up to 10 points on math and coding tasks, indicating the generalizability of the obtained reasoning capabilities ( Figures 2(d) , 2(e) ). We also observed improvements of up to 6 points on natural language inference (NLI) tasks ( Figure 2(f) ). Case analyses suggest that LLMs successfully integrated the commonsense knowledge they had originally acquired during pre-training with the logical reasoning capabilities newly acquired from ALT.

Improvements across various other tasks ( Figure 2(g) ) demonstrate the broad benefits of the obtained reasoning capabilities beyond standard reasoning tasks, though the modest improvements of up to 2 points indicate the need for future research on more effective application of these capabilities.

Our contributions are summarized as follows:

We propose A dditional L ogic T raining (ALT) and empirically verify that it can enhance the reasoning capability of state-of-the-art LLMs across various sizes, from 7B to 70B.
...
[TABLE]
 | \AxiomC The Earth orbits the Sun. \AxiomC If the Earth orbits the sun, the Earth has four seasons. \BinaryInfC The Earth has four seasons. \DisplayProof |  | (3)
[/TABLE]

This step is valid because the conclusion is logically derived from the two premises. Next, consider another logical step:

[TABLE]
 | \AxiomC The Earth orbits the Sun. \AxiomC If the Earth orbits the sun, the Earth does not have four seasons. \BinaryInfC The Earth does not have four seasons. \DisplayProof |  | (4)
[/TABLE]

The second premise and consequently, the conclusion, is factually wrong. Nevertheless, if the premise was hypothetically correct , the conclusion could be logically derived. Therefore, step Equation 4 is also logically valid. Finally:

[TABLE]
 | \AxiomC 1. A Foo star exists. \AxiomC 2. If a Foo star exists, a Bar star also exists. \BinaryInfC A Bar star exists. \DisplayProof |  | (5)
[/TABLE]

“Foo star” and “Bar star” are unknowns; nonetheless, we can still determine that step Equation 5 is logically valid. Steps Equations 3 to 5 above can be abstracted into a deduction rule , i.e., modus ponens, using symbols:

[TABLE]
 | \AxiomC ℱ ℱ \mathcal{F} caligraphic_F \AxiomC ℱ → 𝒢 → ℱ 𝒢 \mathcal{F}\rightarrow\mathcal{G} caligraphic_F → caligraphic_G \RightLabel \stackanchor modus ponens \BinaryInfC 𝒢 𝒢 \mathcal{G} caligraphic_G \DisplayProof |  | (6)
[/TABLE]

As we have seen, the logical validity of a deduction rule depends solely on whether the conclusion is logically derived from the premises, not on the factual correctness of the contents of ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G . Therefore, the contents of ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G can be arbitrary.

Now, we consider what kind of samples would be needed to teach the deduction rule Equation 6 to LLMs. We assume a task to generate the conclusion given the premises as prompt inputs. If the learner were human, they would be able to infer the underlying deduction rule Equation 6 by observing samples such as Equations 3 to 4 . As a result, they would become able to solve the unknown problem Equation 5 .

However, from a purely inductive perspective, samples Equations 3 to 4 cannot simply be generalized to the deduction rule eq. 6 . This is because the samples Equations 3 to 4 themselves do not contain the information that the contents of ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G are arbitrary. In fact, one could generalize samples Equations 3 to 4 to other rules; for example, the conclusion 𝒢 𝒢 \mathcal{G} caligraphic_G can be derived if ℱ ℱ \mathcal{F} caligraphic_F and ℱ → 𝒢 → ℱ 𝒢 \mathcal{F}\rightarrow\mathcal{G} caligraphic_F → caligraphic_G are given as premises and ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G include ’Earth’ as their contents. Innumerable such deduction rules can be inductively inferred from the given samples. In other words, induction has arbitrariness (Hume, 1748 ; Goodman, 1954 ; Quine, 1969 ) .

Humans prefer simpler rules ( Bertrand, ; Wittgenstein, 1922 ) , so they boldly induce up to the deduction rule Equation 6 . However, it is unclear how purely inductive learners such as LLMs, which extract only what can be inferred from samples without prior preferences, induce up to Equation 6 . For example, if only specific contents such as “Alice is kind” and “Bob is smart” are assigned to ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G in training samples, an LLM could develop into a machine that generates the conclusion 𝒢 𝒢 \mathcal{G} caligraphic_G only when the input contains the specific contents. In order for LLMs to accurately induce that ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G are indeed arbitrary:

Design Principle 1 (Reasoning with Unknown Facts). Prepare many samples assigning arbitrary contents to ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G . They will make LLMs accurately induce ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G are indeed arbitrary, ultimately enabling them to reason with unknown facts.
...
[TABLE]
 | \AxiomC ℱ ∧ 𝒢 ℱ 𝒢 \mathcal{F}\land\mathcal{G} caligraphic_F ∧ caligraphic_G \AxiomC ( ℱ ∧ 𝒢 ) → ℋ → ℱ 𝒢 ℋ (\mathcal{F}\land\mathcal{G})\rightarrow\mathcal{H} ( caligraphic_F ∧ caligraphic_G ) → caligraphic_H \BinaryInfC ℋ ℋ \mathcal{H} caligraphic_H \DisplayProof |  | (7)
[/TABLE]

where ∧ \land ∧ denotes logical conjunction, and arbitrary contents are assigned to ℱ , 𝒢 , ℋ ℱ 𝒢 ℋ \mathcal{F},\mathcal{G},\mathcal{H} caligraphic_F , caligraphic_G , caligraphic_H . Suppose that we give this LLM a problem such as:

[TABLE]
 | \AxiomC ℱ ℱ \mathcal{F} caligraphic_F \AxiomC ( ℱ ∧ 𝒢 ) → ℋ → ℱ 𝒢 ℋ (\mathcal{F}\land\mathcal{G})\rightarrow\mathcal{H} ( caligraphic_F ∧ caligraphic_G ) → caligraphic_H \BinaryInfC ? ⁢ ? ? ? ?? ? ? \DisplayProof |  | (8)
[/TABLE]

Since the premises are insufficient for logically deducting the conclusion, outputting nothing is the correct answer.

Unfortunately, an LLM could output ℋ ℋ \mathcal{H} caligraphic_H , which was indeed often observed in our preliminary experiments. This is because while the LLMs can induce from sample Equation 7 that it can generate the conclusion ℋ ℋ \mathcal{H} caligraphic_H when the two premises of Equation 7 are given, the LLMs cannot induce from the sample that it is not allowed to generate the conclusion ℋ ℋ \mathcal{H} caligraphic_H when the premises of Equation 8 are given, as such information is not included in the sample Equation 7 itself. Therefore,

Design Principle 2 (Illogical Reasoning). Include negative samples such as Equation 8 . These samples will make LLMs induce that conclusions cannot be derived from insufficient premises.

### 2.3 Teaching Diverse Reasoning Rules

Deduction rules other than Equation 6 exist:
...
where ∨ \lor ∨ denotes logical disjunction and ¬ \neg ¬ negation. Since there are infinitely many possible logical formulas that can appear as premises and conclusions, there are infinitely many deduction rules. Providing LLMs with these infinite deduction rules is obviously intractable.

Instead of directly providing these infinite deduction rules, we can take another approach. Consider multi-step deductive reasoning ( Figure 2 left), where multiple deduction rules derive a conclusion. Notice that the syllogism in Equation 9 can be expressed by multi-step deductive reasoning using more “atomic” deduction rules. Indeed, there exists a set of atomic deduction rules called the axioms that satisfies the following:

## Theorem 2.1 (Completeness of First-Order Predicate Logic Gödel ( 1930 ) ) .

Any valid deduction rule can be expressed by multistep deductive reasoning constructed from the axioms.
...
In multi-step deductive reasoning, the number of logical steps s 𝑠 s italic_s from premises to a conclusion can vary largely depending on the problem. Therefore:

Design Principle 3’ (Diverse Reasoning Rules). Samples should include diverse numbers of logical steps s 𝑠 s italic_s .

Ideally, this would be sufficient, but empirical evidence has shown that LLMs struggle with constructing multi-step deductive reasoning with large steps s 𝑠 s italic_s (Gontier et al., 2020 ; Morishita et al., 2023 ) . Consequently, LLMs would not excel at handling theorems that require a large number of steps s 𝑠 s italic_s when expressed by the axioms. Therefore, as an additional countermeasure:
...
There are various linguistic structures for expressing the logical relationship ℱ → 𝒢 → ℱ 𝒢 \mathcal{F}\rightarrow\mathcal{G} caligraphic_F → caligraphic_G , such as “If ℱ ℱ \mathcal{F} caligraphic_F then 𝒢 𝒢 \mathcal{G} caligraphic_G ”, “ ℱ ℱ \mathcal{F} caligraphic_F leads to 𝒢 𝒢 \mathcal{G} caligraphic_G ”, and “ ℱ ℱ \mathcal{F} caligraphic_F results in 𝒢 𝒢 \mathcal{G} caligraphic_G ”. If we only include specific expressions in the corpora, LLMs may only learn to react to these specific expressions, which has been observed in previous experiments (Zhang et al., 2022 ; Yuan et al., 2023 ) . To prevent this,
...
RuleTaker ( Clark et al. , 2021 ) (RT) | RuleTaker ( Clark et al. , 2021 ) | (RT) | ≤ \leq ≤ 100 (hand-selected) | ≤ \leq ≤ 100 | (hand-selected) | random formula | random | formula | 2 (implication) | 2 | (implication) | 1–5 | 𝒪 ⁢ ( 1 ) 𝒪 1 \mathcal{O}(1) caligraphic_O ( 1 )
...
2
(implication)
PARARULE-Plus ( Bao et al. , 2022 ) (PRP) | PARARULE-Plus ( Bao et al. , 2022 ) | (PRP) | ≤ \leq ≤ 100 (hand-selected) | ≤ \leq ≤ 100 | (hand-selected) | random formula | random | formula | 2 (implication) | 2 | (implication) | 1–5 | 𝒪 ⁢ ( 1 ) 𝒪 1 \mathcal{O}(1) caligraphic_O ( 1 )
...
2
(implication)
FLD ( Morishita et al. , 2023 ) | ≃ similar-to-or-equals \simeq ≃ 15k (WordNet, subset) | ≃ similar-to-or-equals \simeq ≃ 15k | (WordNet, subset) | random formula | random | formula | 13 (axioms) | 13 | (axioms) | 1–8 | 10 ∼ similar-to \sim ∼ 100
≃ similar-to-or-equals \simeq ≃ 15k
(WordNet, subset)
random
formula
13
(axioms)
FLD ×2 | ≃ similar-to-or-equals \simeq ≃ 100k (WordNet, full) | ≃ similar-to-or-equals \simeq ≃ 100k | (WordNet, full) | adversarial formula | adversarial | formula | ≃ similar-to-or-equals \simeq ≃ 50 (axioms and theorems) | ≃ similar-to-or-equals \simeq ≃ 50 | (axioms and theorems) | 1–8 | 10 ∼ similar-to \sim ∼ 100 (more extensive than FLD) | 10 ∼ similar-to \sim ∼ 100 | (more extensive than FLD)
≃ similar-to-or-equals \simeq ≃ 100k
(WordNet, full)
adversarial
formula
≃ similar-to-or-equals \simeq ≃ 50
(axioms and theorems)
10 ∼ similar-to \sim ∼ 100
(more extensive than FLD)
FLD ×2 ablation corpora → → \rightarrow → | FLD ×2 | ablation corpora → → \rightarrow → | 100 → → \rightarrow → w/o DP1 | 100 | → → \rightarrow → w/o DP1 | not used → → \rightarrow → w/o DP2 | not used | → → \rightarrow → w/o DP2 | 2 (implication) → → \rightarrow → w/o DP3.rules | 2 (implication) | → → \rightarrow → w/o DP3.rules | 1 → → \rightarrow → w/o DP3.steps | 1 | → → \rightarrow → w/o DP3.steps | 1 → → \rightarrow → w/o DP4 | 1 | → → \rightarrow → w/o DP4
FLD ×2
ablation corpora → → \rightarrow →
100
→ → \rightarrow → w/o DP1
not used
→ → \rightarrow → w/o DP2
2 (implication)
→ → \rightarrow → w/o DP3.rules
1
→ → \rightarrow → w/o DP3.steps
1
→ → \rightarrow → w/o DP4
[/TABLE]
Caption: Table 1 : Synthetic logic corpora compared in this study, with their features categorized according to our proposed design principles (DP). Note that the last row of the ablation corpora lists variations of FLD ×2 , each of which differs from the original regarding one of the design principles.

## 3 Creating a Synthetic Corpus based on Design Principles

To prepare diverse samples reflecting the design principles 1 to 4 (DP1-4), we built a novel sample generator by extending the previous one by Morishita et al. ( 2023 ) and then generated the synthetic logic corpus named FLD ×2 (Formal Logic D eduction D iverse). Figure 2 shows a schematic of our generator and a deduction sample. Table 1 compares FLD ×2 with existing corpora. Figure D.3 provides an actual deduction sample included in FLD ×2 .

More specifically, our generator generates deduction samples through the following steps. First, the generator randomly generates a sample of multi-step deductive reasoning written in logical formulas, as shown on the left side of Figure 2 , where a conclusion is derived from premises using multiple deduction rules (See Section D.3 for more details of this generation procedure). At this time, the generator also generates ‘distractor’ logical formulas, which express negative premises of DP2. Next, the generator converts each logical formula into English expressions. To achieve this, the generator first randomly selects a template from pre-defined options, such as “If ℱ ℱ \mathcal{F} caligraphic_F , then 𝒢 𝒢 \mathcal{G} caligraphic_G ,” “ ℱ ℱ \mathcal{F} caligraphic_F leads to 𝒢 𝒢 \mathcal{G} caligraphic_G ,” or “ ℱ ℱ \mathcal{F} caligraphic_F results in 𝒢 𝒢 \mathcal{G} caligraphic_G ,” for the logical formula “ ℱ → 𝒢 → ℱ 𝒢 \mathcal{F}\rightarrow\mathcal{G} caligraphic_F → caligraphic_G .” It then assigns English content randomly constructed from a vocabulary, such as “(that) a Foo star exists” and “(that) a Bar star exists,” to each symbol, such as ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G . Finally, it converts the multi-step deduction into a deduction sample (right side of Figure 2 ) by using the premises as ‘facts’ , the conclusion as ‘hypothesis’ , and the intermediate logical steps as ‘logical steps’ . The deduction sample requires LLMs to generate logical steps that derive a given hypothesis based on the given facts .

Table 1 outlines the comparison of FLD ×2 with other existing corpora (Clark et al., 2021 ; Bao et al., 2022 ; Morishita et al., 2023 ) in terms of DP1-4, which is detailed as follows:

DP1: We assign ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G content randomly constructed from a vocabulary. While the existing corpora used small-sized vocabulary of up to 15k, we use a large vocabulary of around 100k words built from WordNet (Miller, 1995 ) . This will teach LLMs that ℱ ℱ \mathcal{F} caligraphic_F and 𝒢 𝒢 \mathcal{G} caligraphic_G are truly arbitrary, ultimately enabling them to reason with unknown facts.

DP2: The existing corpora used randomly generated logical formulas as distractors. In contrast, we implement adversarial distractors. For example, for a premise ℱ ∧ 𝒢 ℱ 𝒢 \mathcal{F}\land\mathcal{G} caligraphic_F ∧ caligraphic_G , we use ℱ ℱ \mathcal{F} caligraphic_F with missing information (see Equations 7 , 8 ), and for a premise ℱ → ℋ → ℱ ℋ \mathcal{F}\rightarrow\mathcal{H} caligraphic_F → caligraphic_H , we use ℱ ∧ 𝒢 → ℋ → ℱ 𝒢 ℋ \mathcal{F}\land\mathcal{G}\rightarrow\mathcal{H} caligraphic_F ∧ caligraphic_G → caligraphic_H with missing information as distractors. These distractors teach LLMs precisely when a conclusion can and cannot be derived. As with previous corpora, we include a variable number of distractors in each sample, randomly chosen from a range of 0 to 20.

DP3-3”: While the existing corpora used a small number of deduction rules of up to 13 (refer to Figure B.4 of Morishita et al. ( 2023 ) ), we include diverse deduction rules, encompassing the axioms and representative theorems, such as modus ponens, syllogisms, and contraposition, totaling about 50 rules. We include samples with up to s = 8 𝑠 8 s=8 italic_s = 8 logical steps, following (Morishita et al., 2023 ) .

DP4: We manually craft several more English templates per logical formulas than those used in FLD. Since the templates have a nested structure, they yield combinatorially more diverse English expressions. While counting the exact number of the resulting expressions is intractable, we observed at least dozens of expressions per logical formula, including minor variations. See Section D.4 for details.
...
Training Settings: We trained the LLMs by a method similar to supervised fine-tuning; as illustrated in Figure 2 , we used the facts and hypothesis as inputs and logical steps and additional answer label (see Section D.1 ) as outputs. We excluded loss computation for the inputs to prevent LLMs from learning to generate unknown facts. We trained the LLMs for 1 epoch on 100k samples ( ∼ similar-to \sim ∼ 0.1B tokens) from the training split of each corpus, with a batch size of 256, resulting in 390 steps, with a linear warmup for 200 steps. We used the learning rate of 2e-05 for the 8B model and 3e-06 for the 70B model. We used Huggingface (Wolf et al., 2020 ) for implementation.

Prevention of Knowledge Forgetting by Recall Adam Optimizer: Synthetic logic corpora include many samples with unknown facts, so training on them should cause LLMs to forget their knowledge of existing facts. To prevent this, we employed the Recall Adam optimizer (Chen et al., 2020 ) , which regularizes parameter updates to avoid deviating too far from the pre-training parameters. Recall Adam stands out for LLM training for several reasons (see Section E.0.1 for details). We used our re-implemented version 2 2 2 https://github.com/hitachi-nlp/rec-adam . The hyperparameters were: β 1 = 0.9 , β 2 = 0.999 , ϵ = 10 − 6 , fisher coefficient = 4000 formulae-sequence subscript 𝛽 1 0.9 formulae-sequence subscript 𝛽 2 0.999 formulae-sequence italic-ϵ superscript 10 6 fisher coefficient 4000 \beta_{1}=0.9,\beta_{2}=0.999,\epsilon=10^{-6},\text{fisher coefficient}=4000 italic_β start_POSTSUBSCRIPT 1 end_POSTSUBSCRIPT = 0.9 , italic_β start_POSTSUBSCRIPT 2 end_POSTSUBSCRIPT = 0.999 , italic_ϵ = 10 start_POSTSUPERSCRIPT - 6 end_POSTSUPERSCRIPT , fisher coefficient = 4000 for the 8B model and 2000 2000 2000 2000 for the 70B model.
...
Among the LLMs trained with ALT, the one trained on FLD ×2 (i.e., ⊕ direct-sum \oplus ⊕ ALT -FLD ×2 ) achieved the highest generalization performance across the benchmarks. Table 3 shows the performance of the LLMs trained on ablated FLD ×2 corpora, each of which lacks one of the design principles. As seen, ablating any design principle almost always led to performance degradation. These results demonstrate that the proposed design principles are critical to obtaining the maximum possible gain from ALT, and each principle is indispensable.
...
[TABLE]
 ...
 |  |  |  |  |  |  |  | CoT |  | CoT |  | Pro
LLaMA-3.1-8B | 47.9 | 42.8 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 39.6 ± 0.5 plus-or-minus 0.5 {}_{\pm\text{0.5}} start_FLOATSUBSCRIPT ± 0.5 end_FLOATSUBSCRIPT | 35.4 | 65.4 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 60.7 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 44.9 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 61.9 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 8.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 36.5 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 65.3 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 35.8 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-PRP | 48.1 | 43.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 39.2 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 35.7 | 65.6 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 60.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 44.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 61.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 8.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 35.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-RT | 50.1 | 46.8 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 42.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 36.5 | 68.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 63.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 13.7 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 35.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-FLD | 51.9 | 51.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.1 | 70.1 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.9 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 39.6 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT - FLD ×2 | 52.0 | 52.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.0 | 70.7 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
[/TABLE]
Caption: Table 2 : 5-shot performance of LLMs before and after ALT. ⊕ direct-sum \oplus ⊕ ALT - x 𝑥 x italic_x denotes the LLM trained with ALT on the synthetic logic corpus x 𝑥 x italic_x from Table 1 . The color shows the rank in each column (darker is better). Each benchmark set, such as “Logic” and “Math”, comprises various benchmarks in that domain (see Table E.7 ). “Avg.” represents the micro-average of all the benchmarks.

[TABLE]
 | Avg. | Logic | Math | Code | NLI | Others | BBH (3-shot) | BBH (0-shot) | MMLU
 |  |  |  |  |  |  |  | CoT |  | CoT |  | Pro
⊕ direct-sum \oplus ⊕ ALT - FLD ×2 | 52.0 | 52.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.0 | 70.7 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
w/o DP1 | 51.4 | 52.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 39.2 | 70.0 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 59.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
w/o DP2 | 50.6 | 49.9 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.1 | 71.1 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 59.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.6 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 10.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 37.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 35.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
w/o DP3.rules | 50.7 | 50.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 42.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.3 | 69.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 59.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.0 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.8 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
w/o DP3.steps | 51.1 | 51.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.7 | 69.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 59.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.0 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 12.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
w/o DP4 | 51.3 | 52.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 42.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.4 | 70.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 59.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.1 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 12.8 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 39.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
[/TABLE]
Caption: Table 3 : LLaMA-3.1-8B trained on the ablation corpora.
...
LLaMA-3.1-70B | 83.8 ± 1.2 plus-or-minus 1.2 {}_{\pm\text{1.2}} start_FLOATSUBSCRIPT ± 1.2 end_FLOATSUBSCRIPT | 58.9 ± 1.6 plus-or-minus 1.6 {}_{\pm\text{1.6}} start_FLOATSUBSCRIPT ± 1.6 end_FLOATSUBSCRIPT | 34.9 ± 1.1 plus-or-minus 1.1 {}_{\pm\text{1.1}} start_FLOATSUBSCRIPT ± 1.1 end_FLOATSUBSCRIPT | 49.6 ± 0.9 plus-or-minus 0.9 {}_{\pm\text{0.9}} start_FLOATSUBSCRIPT ± 0.9 end_FLOATSUBSCRIPT | 21.5 ± 1.0 plus-or-minus 1.0 {}_{\pm\text{1.0}} start_FLOATSUBSCRIPT ± 1.0 end_FLOATSUBSCRIPT | 64.3 ± 1.2 plus-or-minus 1.2 {}_{\pm\text{1.2}} start_FLOATSUBSCRIPT ± 1.2 end_FLOATSUBSCRIPT | 33.7 ± 0.7 plus-or-minus 0.7 {}_{\pm\text{0.7}} start_FLOATSUBSCRIPT ± 0.7 end_FLOATSUBSCRIPT | 84.0 ± 0.7 plus-or-minus 0.7 {}_{\pm\text{0.7}} start_FLOATSUBSCRIPT ± 0.7 end_FLOATSUBSCRIPT | 85.4 ± 0.9 plus-or-minus 0.9 {}_{\pm\text{0.9}} start_FLOATSUBSCRIPT ± 0.9 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT - FLD ×2 | 83.5 ± 0.5 plus-or-minus 0.5 {}_{\pm\text{0.5}} start_FLOATSUBSCRIPT ± 0.5 end_FLOATSUBSCRIPT | 66.7 ± 0.6 plus-or-minus 0.6 {}_{\pm\text{0.6}} start_FLOATSUBSCRIPT ± 0.6 end_FLOATSUBSCRIPT | 50.9 ± 0.5 plus-or-minus 0.5 {}_{\pm\text{0.5}} start_FLOATSUBSCRIPT ± 0.5 end_FLOATSUBSCRIPT | 81.6 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 25.0 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 69.4 ± 0.5 plus-or-minus 0.5 {}_{\pm\text{0.5}} start_FLOATSUBSCRIPT ± 0.5 end_FLOATSUBSCRIPT | 36.3 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 95.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 85.5 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT
[/TABLE]
Caption: Table 4 : Benchmark-wise 5-shot performance of LLaMA-3.1-70B before and after ALT on FLD ×2 . Refer to Table F.9 for LLaMA-3.1-8B results. Table E.7 details each benchmark.

Figure 2(c) shows that ALT substantially boosted LLaMA-3.1-70B’s performance by up to 30 points on various benchmarks dealing with logical reasoning tasks. Surprisingly, we also observed improvements on abductive reasoning tasks, which go beyond the original deductive reasoning tasks in synthetic logic corpora. Abductive reasoning involves guessing the missing premises that caused the observed conclusion rather than deriving a conclusion from the premises. For example, from the observed conclusion, “the window glass at home was broken and the room was ransacked,” we guess the premise “a burglar broke in.” The improvements would be due to the fact that, while the surface form of abductive reasoning problems differs from that of deductive reasoning, they share the fundamentals of logic reflected in the design principles.

Next, we conduct case analyses to see whether the LLM enhanced by ALT acquired the abilities intended by the proposed design principles (DP1-4). Table 5 shows problems where LLaMA-3.1-70B’s errors have been corrected by ALT. The first problem is very simple, so it is surprising that LLaMA-3.1-70B failed to solve it, indicating the inherent difficulty of learning logical reasoning solely from pre-training. In contrast, ⊕ direct-sum \oplus ⊕ ALT-FLD ×2 , which was additionally trained on FLD ×2 , solved the problem correctly. The premises of the problem are randomly constructed to express unknown facts. Therefore, the result suggests that ⊕ direct-sum \oplus ⊕ ALT-FLD ×2 acquired genuine logical reasoning ability, which can handle unknown facts (DP1).

In the second problem, ⊕ direct-sum \oplus ⊕ ALT-FLD ×2 correctly answered “neutral”, indicating that it successfully learned that conclusions cannot be derived from insufficient facts (DP2).

The third problem comes from the FOLIO benchmark. To solve this problem, LLMs must use syllogism at the first step as follows: “All eels are fish, and no fish are plants. Therefore, all ells are not plants.” ⊕ direct-sum \oplus ⊕ ALT-FLD ×2 answered this problem correctly, suggesting that it successfully learned diverse deduction rules (DP3).

FOLIO problems are created based on Wikipedia topics, describing them in more natural and realistic linguistic expressions than in other benchmarks. As seen in the fourth problem, ⊕ direct-sum \oplus ⊕ ALT-FLD ×2 understands such expressions, suggesting the effect of diverse expressions from DP4 and/or that LLMs can integrate their original linguistic ability with the newly acquired logical reasoning ability.

### 6.2 Math and Coding Tasks

Figures 2(d) , 2(e) shows that ALT substantially boosted the LLaMA-3.1-70B’s performance by up to 7 and 10 points on math and coding tasks, respectively. The math improvements are reasonable, as understanding predicate logic is a prerequisite for solving mathematical problems. For coding, some recent studies have verified the opposite direction, namely, that training on coding data improves logical reasoning abilities (Jiang et al., 2024b ; MA et al., 2024 ; Uchiyama et al., 2024 ) .

### 6.3 NLI Tasks

Figure 2(f) shows that ALT substantially boosted the LLaMA-3.1-70B’s performance by up to 6 points on various natural language inference (NLI) benchmarks. NLI is similar to deductive reasoning in assessing whether a premise supports or contradicts a hypothesis. However, the main difference is that this judgment requires a rich set of commonsense knowledge beyond the given premise.
...
[TABLE]
benchmark | premises | hypothesis | answer (LLaMA-3.1-70B/gold) | answer | (LLaMA-3.1-70B/gold) | required ability | required | ability
answer
(LLaMA-3.1-70B/gold)
required
ability
LogicNLI | Mice are afraid of wolves. Cats are afraid of sheep. Jessica is a cat. Wolves are afraid of cats. Winona is a wolf. Sheep are afraid of cats. | Mice are afraid of wolves. Cats are afraid of sheep. | Jessica is a cat. Wolves are afraid of cats. | Winona is a wolf. Sheep are afraid of cats. | Jessica is afraid of sheep. | Jessica is | afraid of sheep. | neutral/ entailment | neutral/ | entailment | DP1
...
neutral/
entailment
Rhett is not modest. Vivian is confused. Rhett is lazy. If someone is modest or not confused, then he is not eager. | Rhett is not modest. Vivian is confused. | Rhett is lazy. If someone is modest or not confused, then he is not eager. | Rhett is confused. | Rhett is | confused. | entailment/ neutral | entailment/ | neutral | DP2
...
entailment/
neutral
FOLIO | All eels are fish. No fish are plants. Everything displayed in the collection is either a plant or an animal. All animals displayed in the collection are multicellular. A sea eel is displayed in the collection. The sea eel is an eel or an animal or not a plant. | All eels are fish. No fish are plants. | Everything displayed in the collection is either a plant or an animal. | All animals displayed in the collection are multicellular. | A sea eel is displayed in the collection. | The sea eel is an eel or an animal or not a plant. | The sea eel is multicellular or is bacteria. | The sea eel | is multicellular | or is bacteria. | neutral/ entailment | neutral/ | entailment | DP3
...
neutral/
entailment
Common utilities include water, electricity, gas, heating, sewer, trash, and recycling. Many apartment rents cover the cost of water and electricity. Susan lives in an apartment where the rent covers all utilities. The rent of the apartment where Ava lives does not cover any utility expenses. Noah lives in an apartment where the rent does not cover heating. | Common utilities include water, electricity, gas, heating, sewer, trash, and recycling. | Many apartment rents cover the cost of water and electricity. | Susan lives in an apartment where the rent covers all utilities. | The rent of the apartment where Ava lives does not cover any utility expenses. | Noah lives in an apartment where the rent does not cover heating. | Noah and Ava both need to pay the heating bill. | Noah and Ava both | need to pay | the heating bill. | neutral/ entailment | neutral/ | entailment | DP4
...
neutral/
entailment
SNLI | An Indian woman is dancing with her partner. | A woman is moving. | neutral/ entailment | neutral/ | entailment | reasoning with commonsense knowledge
neutral/
entailment
This church choir sings to the masses as they sing joyous songs from the book at a church. | This church choir sings to the masses | as they sing joyous songs from the book at a church. | A choir is singing at a baseball game. | A choir is singing | at a baseball game. | entailment/ contradiction | entailment/ | contradiction
...
contradiction
[/TABLE]
Caption: Table 5 : Problems where LLaMA-3.1-70B initially answered incorrectly and then correctly after training with ALT on FLD ×2 . Red highlights the premises related to the hypothesis.

[TABLE]
benchmark | question | answer
ARC (challenge) | ARC | (challenge) | The end result in the process of photosynthesis is the production of sugar and oxygen. Which step signals the beginning of photosynthesis? | The end result in the process of photosynthesis is the production of | sugar and oxygen. Which step signals the beginning of photosynthesis? | Chlorophyll in the leaf captures light energy. | Chlorophyll in the | leaf captures light energy.
ARC
(challenge)
...
GPQA | A spin-half particle is in a linear superposition 0.8 ⁢ | ↑ ⟩ + 0.6 ⁢ | ↓ ⟩ 0.8 ket ↑ 0.6 ket ↓ 0.8|\uparrow\rangle+0.6|\downarrow\rangle 0.8 | ↑ ⟩ + 0.6 | ↓ ⟩ of its spin-up and spin-down states. If | ↑ ⟩ ket ↑ |\uparrow\rangle | ↑ ⟩ and | ↓ ⟩ ket ↓ |\downarrow\rangle | ↓ ⟩ are the eigenstates of σ z subscript 𝜎 𝑧 \sigma_{z} italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT , then what is the expectation value up to one decimal place, of the operator 10 ⁢ σ z + 5 ⁢ σ x 10 subscript 𝜎 𝑧 5 subscript 𝜎 𝑥 10\sigma_{z}+5\sigma_{x} 10 italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT + 5 italic_σ start_POSTSUBSCRIPT italic_x end_POSTSUBSCRIPT ? | A spin-half particle is in a linear superposition 0.8 ⁢ | ↑ ⟩ + 0.6 ⁢ | ↓ ⟩ 0.8 ket ↑ 0.6 ket ↓ 0.8|\uparrow\rangle+0.6|\downarrow\rangle 0.8 | ↑ ⟩ + 0.6 | ↓ ⟩ of its spin-up | and spin-down states. If | ↑ ⟩ ket ↑ |\uparrow\rangle | ↑ ⟩ and | ↓ ⟩ ket ↓ |\downarrow\rangle | ↓ ⟩ are the eigenstates of σ z subscript 𝜎 𝑧 \sigma_{z} italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT , then what | is the expectation value up to one decimal place, of the operator 10 ⁢ σ z + 5 ⁢ σ x 10 subscript 𝜎 𝑧 5 subscript 𝜎 𝑥 10\sigma_{z}+5\sigma_{x} 10 italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT + 5 italic_σ start_POSTSUBSCRIPT italic_x end_POSTSUBSCRIPT ? | − 0.7 0.7 -0.7 - 0.7
A spin-half particle is in a linear superposition 0.8 ⁢ | ↑ ⟩ + 0.6 ⁢ | ↓ ⟩ 0.8 ket ↑ 0.6 ket ↓ 0.8|\uparrow\rangle+0.6|\downarrow\rangle 0.8 | ↑ ⟩ + 0.6 | ↓ ⟩ of its spin-up
and spin-down states. If | ↑ ⟩ ket ↑ |\uparrow\rangle | ↑ ⟩ and | ↓ ⟩ ket ↓ |\downarrow\rangle | ↓ ⟩ are the eigenstates of σ z subscript 𝜎 𝑧 \sigma_{z} italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT , then what
is the expectation value up to one decimal place, of the operator 10 ⁢ σ z + 5 ⁢ σ x 10 subscript 𝜎 𝑧 5 subscript 𝜎 𝑥 10\sigma_{z}+5\sigma_{x} 10 italic_σ start_POSTSUBSCRIPT italic_z end_POSTSUBSCRIPT + 5 italic_σ start_POSTSUBSCRIPT italic_x end_POSTSUBSCRIPT ?
ARC (challenge) | ARC | (challenge) | Beavers build their homes in ponds and streams. Which characteristic is least critical to building homes in an aquatic environment? | Beavers build their homes in ponds and streams. Which characteristic | is least critical to building homes in an aquatic environment? | (A) waterproof fur (B) webbed hind feet (C) arge, sharp teeth (D) flat, wide tail | (A) waterproof fur (B) webbed hind feet | (C) arge, sharp teeth (D) flat, wide tail
ARC
(challenge)
...
Towards versatile artificial intelligence with reasoning capabilities, we proposed A dditional L ogic T raining on synthetic logic samples. We established systematic design principles well-grounded on symbolic logic theory and previous empirical findings. We constructed a corpus named Formal Logic Deduction Diverse (FLD ×2 ) based on the design principles. We empirically showed that ALT on FLD ×2 substantially enhances the capabilities of state-of-the-art LLMs.
...
Aoki et al. (2024) Yoichi Aoki, Keito Kudo, Tatsuki Kuribayashi, Shusaku Sone, Masaya Taniguchi, Keisuke Sakaguchi, and Kentaro Inui. 2024. First heuristic then rational: Dynamic use of heuristics in language model reasoning .

Askell (2020) Amanda Askell. 2020. Gpt-3: Towards renaissance models. Daily Nous Blog: Philosophers On GPT-3 .
...
Ben Allal et al. (2024) Loubna Ben Allal, Anton Lozhkov, Guilherme Penedo, Thomas Wolf, and Leandro von Werra. 2024. Cosmopedia .
...
Bentivogli et al. (2009) Luisa Bentivogli, Ido Dagan, Hoa Trang Dang, Danilo Giampiccolo, and Bernardo Magnini. 2009. The fifth pascal recognizing textual entailment challenge. In Text Analysis Conference .

Bertolazzi et al. (2024) Leonardo Bertolazzi, Albert Gatt, and Raffaella Bernardi. 2024. A systematic analysis of large language models as soft reasoners: The case of syllogistic inferences .

(12) Russell Bertrand. A history of western philosophy.

Betz et al. (2021) Gregor Betz, Christian Voigt, and Kyle Richardson. 2021. Critical thinking for language models . In Proceedings of the 14th International Conference on Computational Semantics (IWCS) , pages 63–75, Groningen, The Netherlands (online). Association for Computational Linguistics.

Bhagavatula et al. (2019) Chandra Bhagavatula, Ronan Le Bras, Chaitanya Malaviya, Keisuke Sakaguchi, Ari Holtzman, Hannah Rashkin, Doug Downey, Scott Wen-tau Yih, and Yejin Choi. 2019. Abductive commonsense reasoning. arXiv preprint arXiv:1908.05739 .

Bhuiya et al. (2024) Neeladri Bhuiya, Viktor Schlegel, and Stefan Winkler. 2024. Seemingly plausible distractors in multi-hop reasoning: Are large language models attentive readers?
...
Chen et al. (2024) Xinyun Chen, Ryan Andrew Chi, Xuezhi Wang, and Denny Zhou. 2024. Premise order matters in reasoning with large language models . In Proceedings of the 41st International Conference on Machine Learning , volume 235 of Proceedings of Machine Learning Research , pages 6596–6620. PMLR.

Cheng et al. (2017) J. Cheng, M. Bernstein, C. Danescu-Niculescu-Mizil, and J. Leskovec. 2017. Anyone can become a troll: Causes of trolling behavior in online discussions . CSCW: Proceedings of the Conference on Computer-Supported Cooperative Work. Conference on Computer-Supported Cooperative Work, 2017 .
...
Colmerauer and Roussel (1973) A. Colmerauer and P Roussel. 1973. The birth of prolog. The ALP Newsletter .
...
Giampiccolo et al. (2007) Danilo Giampiccolo, Bernardo Magnini, Ido Dagan, and William B Dolan. 2007. The third PASCAL recognizing textual entailment challenge. In ACL-PASCAL Workshop on Textual Entailment and Paraphrasing , pages 1–9.

Gödel (1930) Kurt Gödel. 1930. Uber die vollständigkeit des logikkalküls . Ph.D. thesis, Ph. D. dissertation, University of Vienna.

Gontier et al. (2020) Nicolas Gontier, Koustuv Sinha, Siva Reddy, and Chris Pal. 2020. Measuring systematic generalization in neural proof generation with transformers. Advances in Neural Information Processing Systems , 33:22231–22242.

Goodman (1954) Nelson Goodman. 1954. Fact, fiction, and forecast. london: University of london.

Guiaşu and Tindale (2018) Radu Cornel Guiaşu and Christopher W Tindale. 2018. Logical fallacies and invasion biology. Biology & philosophy , 33(5-6):34.

Habernal et al. (2018) Ivan Habernal, Henning Wachsmuth, Iryna Gurevych, and Benno Stein. 2018. The argument reasoning comprehension task: Identification and reconstruction of implicit warrants . In Proceedings of the 2018 Conference of the North American Chapter of the Association for Computational Linguistics: Human Language Technologies, Volume 1 (Long Papers) , pages 1930–1940, New Orleans, Louisiana. Association for Computational Linguistics.
...
Han et al. (2022) Simeng Han, Hailey Schoelkopf, Yilun Zhao, Zhenting Qi, Martin Riddell, Luke Benson, Lucy Sun, Ekaterina Zubova, Yujie Qiao, Matthew Burtell, et al. 2022. Folio: Natural language reasoning with first-order logic. arXiv e-prints , pages arXiv–2209.

Hansson (2004) Sven Ove Hansson. 2004. Fallacies of risk. Journal of Risk Research , 7(3):353–360.
...
Hume (1748) David Hume. 1748. An enquiry concerning human understanding (section iv). Recuperado de http://www. clorenzano. com. ar .
...
Liu et al. (2023a) Hanmeng Liu, Jian Liu, Leyang Cui, Zhiyang Teng, Nan Duan, Ming Zhou, and Yue Zhang. 2023a. Logiqa 2.0—an improved dataset for logical reasoning in natural language understanding . IEEE/ACM Transactions on Audio, Speech, and Language Processing , 31:2947–2962.
...
Liu et al. (2023c) Hanmeng Liu, Zhiyang Teng, Leyang Cui, Chaoli Zhang, Qiji Zhou, and Yue Zhang. 2023c. LogiCoT: Logical chain-of-thought instruction tuning . In Findings of the Association for Computational Linguistics: EMNLP 2023 , pages 2908–2921, Singapore. Association for Computational Linguistics.
...
Liu et al. (2023d) Jiawei Liu, Chunqiu Steven Xia, Yuyao Wang, and Lingming Zhang. 2023d. Is your code generated by chatGPT really correct? rigorous evaluation of large language models for code generation . In Thirty-seventh Conference on Neural Information Processing Systems .
...
MA et al. (2024) YINGWEI MA, Yue Liu, Yue Yu, Yuanliang Zhang, Yu Jiang, Changjian Wang, and Shanshan Li. 2024. At which training stage does code data help LLMs reasoning? In The Twelfth International Conference on Learning Representations .
...
McCarthy (1959) John W. McCarthy. 1959. Programs with common sense. In Proc. Tedding Conf. on the Mechanization of Thought Processes , pages 75–91.
...
Mitchell (2023) Melanie Mitchell. 2023. Can large language models reason? blog , pages https://aiguide.substack.com/p/can–large–language–models–reason.
...
Mondorf and Plank (2024) Philipp Mondorf and Barbara Plank. 2024. Liar, liar, logical mire: A benchmark for suppositional reasoning in large language models .
...
Nafar et al. (2024) Aliakbar Nafar, K. Brent Venable, and Parisa Kordjamshidi. 2024. Teaching probabilistic logical reasoning to transformers . In Findings of the Association for Computational Linguistics: EACL 2024 , pages 1615–1632, St. Julian’s, Malta. Association for Computational Linguistics.
...
Paglieri (2017) Fabio Paglieri. 2017. A plea for ecological argument technologies . Philosophy & Technology , 30(2):209–238.
...
Quine (1969) Willard Van Orman Quine. 1969. Epistemology naturalized. ontological relativity and other essays. New York: Columbia UP .
...
Sanyal et al. (2022a) Soumya Sanyal, Zeyi Liao, and Xiang Ren. 2022a. Robustlr: Evaluating robustness to logical perturbation in deductive reasoning. arXiv preprint arXiv:2205.12598 .

Sanyal et al. (2022b) Soumya Sanyal, Harman Singh, and Xiang Ren. 2022b. Fairr: Faithful and robust deductive reasoning over natural language. In Proceedings of the 60th Annual Meeting of the Association for Computational Linguistics (Volume 1: Long Papers) , pages 1075–1093.

Shortliffe (1976) eh Shortliffe. 1976. Computer based medical consultations: Mycin. Elsevier .
...
Sprague et al. (2024) Zayne Rea Sprague, Xi Ye, Kaj Bostrom, Swarat Chaudhuri, and Greg Durrett. 2024. MuSR: Testing the limits of chain-of-thought with multistep soft reasoning . In The Twelfth International Conference on Learning Representations .

Sunstein and Hastie (2015) Cass R Sunstein and Reid Hastie. 2015. Wiser: getting beyond groupthink to make groups smarter . Harvard Business Review Press, Boston.
...
Tian et al. (2021) Jidong Tian, Yitian Li, Wenqing Chen, Liqiang Xiao, Hao He, and Yaohui Jin. 2021. Diagnosing the first-order logical reasoning ability through logicnli. In Proceedings of the 2021 Conference on Empirical Methods in Natural Language Processing , pages 3738–3747.
...
Wittgenstein (1922) Ludwig Wittgenstein. 1922. Tractatus Logico Philosophicus: Logical-Philosophical Treatise . Really Simple Media.
...
Wu et al. (2023) Zhaofeng Wu, Linlu Qiu, Alexis Ross, Ekin Akyürek, Boyuan Chen, Bailin Wang, Najoung Kim, Jacob Andreas, and Yoon Kim. 2023. Reasoning or reciting? exploring the capabilities and limitations of language models through counterfactual tasks .
...
Young et al. (2022) Nathan Young, Qiming Bao, Joshua Bensemann, and Michael J Witbrock. 2022. Abductionrules: Training transformers to explain unexpected inputs. In Findings of the Association for Computational Linguistics: ACL 2022 , pages 218–227.
...
Yuan et al. (2023) Zhangdie Yuan, Songbo Hu, Ivan Vulić, Anna Korhonen, and Zaiqiao Meng. 2023. Can pretrained language models (yet) reason deductively? In Proceedings of the 17th Conference of the European Chapter of the Association for Computational Linguistics , pages 1439–1454.
...
Zhou et al. (2024a) Jin Peng Zhou, Charles E Staats, Wenda Li, Christian Szegedy, Kilian Q Weinberger, and Yuhuai Wu. 2024a. Don’t trust: Verify – grounding LLM quantitative reasoning with autoformalization . In The Twelfth International Conference on Learning Representations .
...
Zhu et al. (2024) Kaijie Zhu, Jiaao Chen, Jindong Wang, Neil Zhenqiang Gong, Diyi Yang, and Xing Xie. 2024. Dyval: Dynamic evaluation of large language models for reasoning tasks . In The Twelfth International Conference on Learning Representations .
...
[FIGURE: image not rendered]
Caption: Figure D.3 : A real deduction sample included in Formal Logic Deduction Diverse. Facts and hypothesis are given to LLMs, then the LLMs are required to generate logical steps to (dis-)prove the hypothesis based on the facts, and an answer label (see Section D.2 ).

### D.4 Linguistic Expressions

We prepared linguistic templates for each logical formula, exemplified as follows: {alignSmall} ⟨(A ∧B) →C⟩: & If ⟨(A ∧B) .predicate_phrase ⟩, then ⟨C .predicate_phrase ⟩. : ⟨(A ∧B). noun_pharse ⟩ ⟨ cause_synonyms ⟩ ⟨C .noun_phrase ⟩. : (…) ⟨(A ∧B) .predicate_phrase ⟩: A ⟨ occur_synonyms ⟩ and also B ⟨ occur_synonyms ⟩. : A and also B ⟨ occur_synonyms ⟩. : Both A and B ⟨ occur_synonyms ⟩. : (…) ⟨C .predicate_phrase ⟩: C ⟨ occur_synonyms ⟩. : (…) ⟨ occur_synonyms ⟩: occur : happen : take place : (…) ⟨(A ∧B). noun_pharse ⟩: A and B : A and also B : Both A and B : That A and B ⟨ occur_synonyms ⟩ : (…) ⟨ cause_synonyms ⟩: cause : result in : lead to : bring about : (…) ( …) As can be seen, the templates can be nested deeply, yielding combinatorially diverse linguistic expressions.
...
[TABLE]
Set | Benchmarks | Reasoning form | Reasoning | form | Required knowledge | Required | knowledge
Reasoning
form
Required
knowledge
Logic | bAbi deduction ( Weston et al. , 2015 ) , | deduction | - (not required)
FOLIO ( Han et al. , 2022 )
LogicNLI ( Tian et al. , 2021 )
RobustLR ( Sanyal et al. , 2022a )
AR-LSAT ( Zhong et al. , 2021 ) | commonsense
...
NLI | HELP ( Yanaka et al. , 2019 ) | validate a conclusion based on given premises | commonsense
MultiNLI ( Williams et al. , 2018 )
RTE ( Dagan et al. , 2005 ; Giampiccolo et al. , 2007 ; Bentivogli et al. , 2009 )
SNLI ( Bowman et al. , )
Math | GSM8k ( Cobbe et al. , 2021 ) | Math | Math
MATH ( Hendrycks et al. , 2021b )
MathQA ( Amini et al. , 2019 )
Coding | HumanEval ( Chen et al. , 2021 ) | Coding | Coding
MBPP ( Austin et al. , 2021 )
MBPP+ ( Liu et al. , 2023d )
MultiPL-E (cpp/go) ( Cassano et al. , 2023 )
Others | CommonsenseQA ( Talmor et al. , 2018 ) | complicated procedures | commonsense
HellaSWAG ( Zellers et al. , 2019 )
SQuAD2 ( Rajpurkar et al. , 2018 )
WinoGrande ( Sakaguchi et al. , 2021 )
ARC (easy/challenge) ( Clark et al. , 2018 ) | science
GPQA ( Rein et al. , 2023 )
OpenBookQA ( Mihaylov et al. , 2018 )
SciQ ( Welbl et al. , 2017 )
aggregated | MMLU ( Hendrycks et al. , 2021a ) | various | various
MMLU-Pro ( Wang et al. , 2024c )
BBH ( Suzgun et al. , 2022 )
[/TABLE]
Caption: Table E.7 : 31 benchmarks used in the experiments. These benchmarks cover a wide range of tasks and are prominent for LLM evaluation. We also show the form of reasoning and the type of knowledge required to solve the problems in each benchmark.
...
[TABLE]
 ...
 |  |  |  |  |  |  |  | CoT |  | CoT |  | Pro
LLaMA-3.1-8B | 47.9 | 42.8 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 39.6 ± 0.5 plus-or-minus 0.5 {}_{\pm\text{0.5}} start_FLOATSUBSCRIPT ± 0.5 end_FLOATSUBSCRIPT | 35.4 | 65.4 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 60.7 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 44.9 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 61.9 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 8.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 36.5 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 65.3 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 35.8 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-PRP w/o RecAdam w/o RecAdam {}_{\text{w/o RecAdam}} start_FLOATSUBSCRIPT w/o RecAdam end_FLOATSUBSCRIPT | 43.5 | 39.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 29.1 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 35.3 | 57.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 61.0 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 40.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 47.0 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 3.9 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 6.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 64.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 34.0 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-PRP | 48.1 | 43.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 39.2 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 35.7 | 65.6 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 60.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 44.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 61.8 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 8.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 35.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-RT | 50.1 | 46.8 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 42.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 36.5 | 68.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 63.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 13.7 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 35.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT-FLD | 51.9 | 51.6 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.1 | 70.1 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 64.9 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.9 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 39.6 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.4 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT - FLD ×2 | 52.0 | 52.2 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 43.2 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 38.0 | 70.7 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 61.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 46.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.3 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 11.3 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 38.7 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 65.5 ± 0.1 plus-or-minus 0.1 {}_{\pm\text{0.1}} start_FLOATSUBSCRIPT ± 0.1 end_FLOATSUBSCRIPT | 36.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT
[/TABLE]
Caption: Table F.8 : 5-shot performance of LLMs before and after ALT. ⊕ direct-sum \oplus ⊕ ALT - x 𝑥 x italic_x denotes the LLM trained with ALT on the synthetic logic corpus x 𝑥 x italic_x from Table 1 . Color shows the rank in each column (darker is better). “Logic”, “Math”, “Code”, and “Others” each comprises various benchmarks (see Table E.7 ). “Avg.” represents the micro-average of all the benchmarks. “w/o RecAdam” denotes that LLM was trained without knowledge forgetting prevention by Recall Adam optimizer.

[TABLE]
 | bAbiD | FOLIO | LogicNLI | RobustLR | AR-LSAT | LogiQA | ReClor | AbductionR | ART
LLaMA-3.1-8B | 48.7 ± 1.6 plus-or-minus 1.6 {}_{\pm\text{1.6}} start_FLOATSUBSCRIPT ± 1.6 end_FLOATSUBSCRIPT | 50.0 ± 1.6 plus-or-minus 1.6 {}_{\pm\text{1.6}} start_FLOATSUBSCRIPT ± 1.6 end_FLOATSUBSCRIPT | 28.5 ± 1.0 plus-or-minus 1.0 {}_{\pm\text{1.0}} start_FLOATSUBSCRIPT ± 1.0 end_FLOATSUBSCRIPT | 43.2 ± 0.9 plus-or-minus 0.9 {}_{\pm\text{0.9}} start_FLOATSUBSCRIPT ± 0.9 end_FLOATSUBSCRIPT | 20.7 ± 1.0 plus-or-minus 1.0 {}_{\pm\text{1.0}} start_FLOATSUBSCRIPT ± 1.0 end_FLOATSUBSCRIPT | 39.6 ± 1.2 plus-or-minus 1.2 {}_{\pm\text{1.2}} start_FLOATSUBSCRIPT ± 1.2 end_FLOATSUBSCRIPT | 28.7 ± 0.7 plus-or-minus 0.7 {}_{\pm\text{0.7}} start_FLOATSUBSCRIPT ± 0.7 end_FLOATSUBSCRIPT | 52.4 ± 0.9 plus-or-minus 0.9 {}_{\pm\text{0.9}} start_FLOATSUBSCRIPT ± 0.9 end_FLOATSUBSCRIPT | 73.4 ± 1.1 plus-or-minus 1.1 {}_{\pm\text{1.1}} start_FLOATSUBSCRIPT ± 1.1 end_FLOATSUBSCRIPT
⊕ direct-sum \oplus ⊕ ALT - FLD ×2 | 55.8 ± 0.6 plus-or-minus 0.6 {}_{\pm\text{0.6}} start_FLOATSUBSCRIPT ± 0.6 end_FLOATSUBSCRIPT | 54.5 ± 0.6 plus-or-minus 0.6 {}_{\pm\text{0.6}} start_FLOATSUBSCRIPT ± 0.6 end_FLOATSUBSCRIPT | 42.0 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 62.6 ± 0.3 plus-or-minus 0.3 {}_{\pm\text{0.3}} start_FLOATSUBSCRIPT ± 0.3 end_FLOATSUBSCRIPT | 21.1 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 42.8 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT | 29.4 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 85.5 ± 0.2 plus-or-minus 0.2 {}_{\pm\text{0.2}} start_FLOATSUBSCRIPT ± 0.2 end_FLOATSUBSCRIPT | 76.1 ± 0.4 plus-or-minus 0.4 {}_{\pm\text{0.4}} start_FLOATSUBSCRIPT ± 0.4 end_FLOATSUBSCRIPT
[/TABLE]
Caption: Table F.9 : Benchmark-wise 5-shot performance of LLaMA-3.1-8B before and after ALT on FLD ×2 .
...
Justification: Sections 4 , E . Further, we release all the resources, including (i) the corpus, (ii) the trained model, and (iii) code for corpus generation, LLM training, and LLM evaluation 3 3 3 https://anonymous.4open.science/r/ALT/README.md .
