...
authors: Michel Wong, Ali Alshehri, Sophia Kao, Haotian He (Apple)
venue: "arXiv preprint (v1 submitted 2025-11-05). Comments field: EMNLP 2025 Industry Track (per WebFetch summary of the abs page); 9 pages including appendix. cs.CL, cs.LG."
fetch_route: "curl -sL https://arxiv.org/html/2511.03080 (LaTeXML HTML, v1, 120KB) -> local parse with python3 + BeautifulSoup (html.parser)"
...
- Expected citation: "Wong et al., 2025", arXiv ID 2511.03080. This matches exactly: title, authors (Michel Wong, Ali Alshehri, Sophia Kao, Haotian He, all affiliated with Apple), and arXiv ID all confirmed via both WebFetch (metadata only) and the raw LaTeXML HTML source (used for body text). No ID-resolution discrepancy.
- Venue: WebFetch's summary of the abs page reported "EMNLP 2025 Industry Track" as the comments/venue field. This was not independently re-derived from the raw HTML body (the HTML source does not repeat the venue string in the body text extracted), so it is reported here as coming from the abs-page metadata fetch only, per the instructions to use WebFetch for metadata.
- Only one version (v1, 2025-11-05) was found; the base href in the fetched HTML confirms `/html/2511.03080v1/`.
- No other discrepancies identified.

# Extraction notes

- Route 1 (WebFetch on the abs page) succeeded and was used only for metadata (title, authors, abstract summary, venue/comments), per instructions.
- Route 2 (`curl -sL https://arxiv.org/html/2511.03080`) succeeded on the first try: returned a full LaTeXML-generated HTML document (~120KB) with real body text, section structure, 6 tables, 1 figure, footnotes, and full bibliography. No fallback routes (ar5iv, e-print LaTeX source, or PDF) were needed.
- Parsed locally with Python 3 + BeautifulSoup (`html.parser` backend; installed via `pip install --break-system-packages beautifulsoup4` since bs4 was not preinstalled). Custom script walked the `<article>` tag's section tree in document order, rendering paragraphs, tables (as pipe-delimited rows preceded by `[TABLE] <caption>`), the one figure (as `[FIGURE] <caption>` — the image itself is not reproduced, only its caption, since this is a text extraction), and footnotes (collected into an endnotes list at the bottom, de-duplicated, with inline `[fnN]` markers left in place of the original in-place footnote text to avoid duplicating footnote prose inside body paragraphs).
- Self-check: verified 3 verbatim sentences from 3 different sections (Introduction, Discussion §5.2 Future Work, and Acknowledgement) against the raw fetched HTML via `grep -o`, and all three matched exactly — confirming the extraction is genuine verbatim prose, not paraphrase.
- Missing/unrendered: Figure 1's actual image is not included (only its caption is preserved, marked `[FIGURE]`); this is explicitly noted rather than reconstructed. All 6 tables' cell contents were captured in full and appear complete.

---

# PolyNorm: Few-Shot LLM-Based Text Normalization for Text-to-Speech

Michel Wong, Ali Alshehri, Sophia Kao, Haotian He {michel.wong, a_alshehri, sophia_kao, haotian}@apple.com Apple Correspondence: michel.wong@apple.com
...
Orig: Le taux de change est de 1€ = 1,10$. Norm: Le taux de change est de un euro égale un virgule dix dollars.

Orig: El 1º de Enero. Norm: El primero de Enero.

Orig: 容量は500mL。 Norm: ヨウリョウはゴヒャクミリリットル。
...
Orig: 05/20/2023 Norm (accepted): may twentieth twenty twenty three Norm (rejected): the twentieth of may two thousand and twenty three
...
We evaluated GPT-4o-mini and GPT-4o as candidate systems against Apple Siri production rule-based normalization baseline system. Because it is proprietary, we cannot release code or provide external access. However, to help readers judge strength and fairness, we report baseline scores for a representative subset of seven categories across all eight languages in Table 4 below [fn3: Table 3 in Appendix B shows side-by-side examples illustrating some incorrect baseline outputs]. It is important to note that the baseline predates our benchmark and was not tuned to it.
...
**[TABLE] Table 2: WER and BLEU scores across 8 languages using PolyNorm-Benchmark. Best scores per row are in bold.**
...
(Note: original table marks "Best scores per row are in bold" — bolding not preserved in this plain-text/markdown-table extraction; all numeric values transcribed verbatim from the source table.)
...
- Facebook AI (2022) Facebook AI. 2022. Nllb-200 distilled (1.3b). https://huggingface.co/facebook/nllb-200-distilled-1.3B.
- Graliński et al. (2006) Filip Graliński, Krzysztof Jassem, Agnieszka Wagner, and Mikołaj Wypych. 2006. Text normalization as a special case of machine translation. In Proceedings of the International Multiconference on Computer Science and Information Technology, pages 51–56.
- Mosquera et al. (2012) Alejandro Mosquera, Elena Lloret, and Paloma Moreda. 2012. Towards facilitating the accessibility of web 2.0 texts through text normalisation. In Proceedings of the First Workshop on Speech and Language Processing for Assistive Technologies, pages 9–17.
- Poświata and Perełkiewicz (2019) Rafał Poświata and Michał Perełkiewicz. 2019. Numbers normalisation in the inflected languages: a case study of polish. In Proceedings of the 7th Workshop on Balto-Slavic Natural Language Processing, pages 23–28.
- Sproat et al. (2001) Richard Sproat, Alan W Black, Stanley Chen, Shankar Kumar, Mari Ostendorf, and Christine D Richards. 2001. Normalization of non-standard words. Computer Speech and Language, 15(3):287–333.
...
- Zhang et al. (2024) Yang Zhang, Travis M. Bartley, Mariana Graterol-Fuenmayor, Vitaly Lavrukhin, Evelina Bakhturina, and Boris Ginsburg. 2024. A chat about boring problems: Studying gpt-based text normalization. Preprint, arXiv:2309.13426.
...
### Appendix A Instruction Prompt Template
...
### Appendix B Baseline Normalization Examples

**[TABLE] Table 3: Examples of baseline TN errors across the targeted languages and selected categories.**

| Language | Category | Example | Ground Truth | Error |
|---|---|---|---|---|
| German | Address | Die Wohnung befindet sich in der Hauptstraße 45. | die wohnung befindet sich in der hauptstrasse fünfundvierzig. | die wohnung befindet sich in der hauptstrasse fünf und vierzig. |
| American English | Legal References | The regulation is 15 CFR Part 12. | the regulation is fifteen c f r part twelve. | the regulation is fifteen cfr part twelve. |
| Mexican Spanish | Phone Numbers | Mi celular es 442-789-0123. | mi celular es cuatro cuatro dos, siete ocho nueve, cero uno dos tres. | mi celular es cuatrocientos cuarenta y dos, setecientos ochenta y nueve, cero un veintitrés. |
| French | Sports Scores | Le tennis de table a fini 11-9. | le tennis de table a fini onze à neuf. | le tennis de table a fini onze moins neuf. |
| Italian | Currencies | La tariffa è CHF 20. | la tariffa è venti franchi svizzeri. | la tariffa è c h f venti. |
| Lithuanian | Mathematical Expressions | 5 × 6 = 30. | penki kart šeši lygu trisdešimt. | penki kart šeši yra trisdešimt. |
| Mandarin Chinese | Musical Notation | 速度标记𝅘𝅥=120 | 速度标记四分音符等于一百二十 | 速度 标记 = 一 百 二 十 |
| Japanese | Currencies | 価格は₩15,000。 | カカクワ イチマン ゴセンワン。 | カカク ハ イチマン ゴセンワン。 |

### Appendix C Baseline model WER (%) by Language and Selected Categories

The Overall column reports the average WER across all 27 categories.

**[TABLE] Table 4: Baseline model WER results by language and category**
...
### Appendix D GPT-4o WER (%) by Language and Selected Categories

The Overall column reports the average WER across all 27 categories.

#### D.1 Iteration 2

**[TABLE] Table 5: GPT-4o WER results for Iteration 2**
...
**[TABLE] Table 6: GPT-4o WER results for Iteration 3**
...
[1] "È il 17° secolo." translates to "It's the 17th century." / "Estoy en el 17º piso." translates to "I am on the 17th floor." (this footnote appeared as two related sentences in the source, attached to the Figure 1 reference in the Introduction)
