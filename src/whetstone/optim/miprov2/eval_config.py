...
from whetstone.eval import SeedPlan, SamplingConfig, TaskSet
...
MIPROV2_EVAL_EXECUTION_POLICY_SCHEMA = (
    "whetstone.miprov2_evaluation_execution_policy"
)
MIPROV2_EVAL_EXECUTION_POLICY_SCHEMA_VERSION = 1
...
class Miprov2EvaluationExecutionPolicy(BaseModel):
    ...
    def identity_hash(self) -> str:
        return compute_identity_hash(
            schema=MIPROV2_EVAL_EXECUTION_POLICY_SCHEMA,
            schema_version=(
                MIPROV2_EVAL_EXECUTION_POLICY_SCHEMA_VERSION
            ),
...
class EvalBindingRequest(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
    ...
    @model_validator(mode="after")
    def _validate_request(self) -> EvalBindingRequest:
        require_full_hash(
        ...
        if not self.task_batch_hashes or self.num_seeds <= 0:
            raise ValueError(
...
class EvalBinding(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_binding(self) -> EvalBinding:
        ...
        if (
            self.seed_plan.task_hashes != tasks
            or self.seed_plan.num_seeds != self.request.num_seeds
        ):
            raise ValueError("bound TaskTrial Plan conflicts with request")
        task_set_hash = self.task_set.identity_hash()
        seed_plan_hash = self.seed_plan.identity_hash()
        require_full_hash(task_set_hash, field="task_set.identity_hash")
        require_full_hash(
            seed_plan_hash,
            field="seed_plan.identity_hash",
        )
        expected_assignment = (
    ...
    def identity_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "seed_plan": self.seed_plan.model_dump(mode="json"),
...
class EvalBindingResolver(Protocol):
    def resolve(
        self,
        request: EvalBindingRequest,
    ) -> EvalBinding: ...
