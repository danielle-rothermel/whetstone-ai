...
A live validation run drives many cells, often concurrently against one shared
key. This module is the ONE structured event stream a dumb watcher can wake on:
the runner appends typed JSONL events to ``<root>/logs/events.jsonl`` (run-root
level, append-only, one stream keyed by cell/screen id) the moment something
meaningful happens, and mirrors each event with a loud plain-text stderr marker
so ``grep``-based watchers see the same signal.

The event types, all pushed the instant the runner observes them:

* :data:`RATE_LIMIT_PRESSURE` -- heartbeat-window counters of 429s, rate-limit
  retries, and concurrency-halved incidents, emitted only when nonzero.
...
* :data:`CELL_FINALIZED` -- a cell reached a terminal statistical status
  (status, delta and CI when applicable, realized spend, duration).
* :data:`CELL_FAILED` -- a cell failed with a typed failure reason, never a
  bare string dump of an exception.
* :data:`ARM_INCOMPLETE` -- the incomplete-official-arm transient-failure
  class.
...
* **estimates are labeled** -- heartbeat spend estimates over-attribute under
  ...
  ``spend_estimate_usd`` and are never presented as realized spend. A finalized
...
write is swallowed loudly, because a broken event stream cannot break a run.
"""
...
#: Persisted-format contract: the event-schema tag stamped on every row. A
#: structured reader keys off it so the envelope can evolve without silently
#: breaking downstream joins.
EVENTS_SCHEMA = "whetstone.runner.events/v1"

#: Persisted-format contract: the typed event-name constants, which are the
#: ``event`` field's closed value set.
...
#: The loud plain-text stderr marker mirroring each event. Each marker is a
#: stable, unique, greppable token a watcher can ``grep -m1`` on.
...
def is_rate_limit_code(code: str) -> bool:
    ...
    lowered = code.lower()
    return (
        "429" in lowered or "rate_limit" in lowered or "rate-limit" in lowered
    )
...
class EventUnit(BaseModel):
    ...
    The composite ``cell_id`` (``opt:env:aN``) and ``screen_id`` display
    strings are recorded alongside the components, but a downstream reader
    joins on the separate fields (``env`` / ``optimizer`` / ``attempt`` /
    ``lane`` / ``model``) so it never has to parse the composite id and an id
    format change cannot break its joins. Every component is ``None`` when it
    does not apply -- a screen-level event has no ``attempt``, a pre-model
    event has no ``model`` -- null-not-zero, never a placeholder.
    """
...
class RunEvent(BaseModel):
    ...
    timestamp, never an empty string. ``unit`` carries the structured identity
    so a reader joins on components rather than a parsed id string. ``marker``
    is the loud plain-text token mirrored to stderr. ``fields`` holds the
    event-type payload, already truthfulness-checked by the builder.
    ``extra="forbid"`` keeps the envelope typed; the open ``fields`` dict is
    where per-type data lives.
    """
...
def rate_limit_pressure_event(
    *,
    ...
    """A heartbeat-window rate-limit-pressure event.
    ...
    because it observed pressure on the shared key. ``concurrency_halved`` and
    ``guard_timeouts`` mirror the fan-out backpressure signals. The caller
    emits this only when at least one counter is nonzero.
    """
    return _event(
        RATE_LIMIT_PRESSURE,
        at=at or _utc_now(),
...
def attempt_skipped_event(
    *,
    ...
    """An event for a skipped already-completed attempt.

    Skipping is correct for wave relaunches; the event makes it loud so a
    watcher never mistakes the runner's echo of the prior line's stats for
    fresh work. ``prior_status`` is the completed record's status.
    """
    return _event(
        ATTEMPT_SKIPPED,
        at=at or _utc_now(),
...
def cell_finalized_event(
    *,
    ...
    ``delta`` and ``delta_ci95`` are ``None`` when the cell has no delta, for
    example an eval-only row or a cut-short cell -- null-not-zero.
    ``realized_spend_usd`` is the cell's own credits delta, never a heartbeat
    estimate; ``None`` when spend is unknown (a non-OpenRouter lane or an
    unbounded gap).
    """
...
def cell_failed_event(
    *,
    ...
    """A cell failed with a typed failure reason.

    ``reason_class`` is the failure's type name; ``detail`` is a bounded human
    message. A watcher keys off ``reason_class``, not the free-form detail.
    """
    return _event(
        CELL_FAILED,
        at=at or _utc_now(),
...
def arm_incomplete_event(
    *,
    ...
    ``detail`` names the failed arms and row accounting exactly as the ledger
    note does; the cell is not a certified result, and a re-run supersedes it.
    """
    return _event(
        ARM_INCOMPLETE,
        at=at or _utc_now(),
...
def latency_snapshot_event(
    *,
    ...
    The model is on ``unit.model``, a structured field rather than something
    parsed from an id. ``median_latency_s`` is ``None`` when no call in the
    window reported a latency (null-not-zero); ``coverage`` is the number of
    rows the median was computed over, so a partial-coverage window is never
...
def traceback_event(
    *,
    ...
    both the marker line and the structured field. Emitted from the unhandled
    boundary, after which the exception re-raises so the process still fails
    loudly.
    """
    return _event(
        TRACEBACK,
...
class EventStream:
    ...
    Constructed over the run root; the stream lives at
    ``<root>/logs/events.jsonl`` -- one shared stream for the whole run, keyed
    by the cell and screen id fields. :meth:`emit` appends one event's JSON
    line (flushed so a crash keeps it) and mirrors its loud marker to stderr,
    so a ``grep``-based watcher on the plain log and a structured JSONL reader
    both see it.
    ...
    ``emit`` opens, writes, and closes a single ``O_APPEND`` line, which POSIX
    writes atomically for a line this small -- the same single-line-append
    discipline the partial log relies on.

    Emission is best-effort: a write failure is swallowed, loudly to stderr, so
    a broken event stream can never break a cell's actual work.
    """
    ...
    def emit(self, event: RunEvent) -> None:
        ...
        """
        # The loud marker always fires -- it is the greppable signal, and the
        # cheapest half -- so do it first and independently.
        try:
...
def emit_traceback_on_unhandled(
    ...
    *,
    ...
    without emitting: the runner's own typed, already-handled failures, which
    the CLI reports itself and which are not unhandled.
