...
It reduces to ONE aggregate -- the Average Binary Test Pass Rate (the plain
Reward-bearing metric; NOT the ed1/ed1m weighted blend) -- using the same
two-stage mean the QA/ed1 paths use, and returns a
:class:`~whetstone.runner.eval_run.SplitEvaluation` whose ``score`` is the pass
rate. The frozen input-arm construction REUSES the screen driver verbatim
(``split_prompt`` / ``_direct_body`` / ``renamed_task``), so a d1 EVAL anchor
on an arm reproduces the corresponding screen arm's numbers. Nothing here makes
a
live paid call by itself: the transport and code-eval scorer are injected.
"""
...
from whetstone.envs.ed1 import ed1_reward_from_pass_rate
...
def _input_arm_text(
    ...
    body = _direct_body(f"direct_{arm}", parts, rename_token=token)
    score_task = (
        renamed_task(ht, old=ht.entry_point, new=token)
        if arm == "renamed" else ht
    )
...
def _drive_row(
    *,
    ...
    if code_score.infrastructure_unknown:
        return _D1RowOutcome(
            pass_value=None, output_text=output_text,
...
def run_d1_eval(
    ...
    *,
    ...
    pass_aggregate = _aggregate_metric(
        name=D1_PASS_RATE_NAME, graph_hash=graph_hash,
