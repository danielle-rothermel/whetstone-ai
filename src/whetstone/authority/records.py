...
class PlannedKeyResult(BaseModel):
    ...
    result_ref: TypedContentRef | None = None
...
class CompletenessDecision(BaseModel):
    ...
    def _validate(self) -> CompletenessDecision:
        ...
        if self.complete != computed_complete:
            raise ValueError(
                "complete must be true iff missing_count == 0"
            )
        if self.certified and not self.complete:
            raise ValueError(
                "an incomplete evaluation cannot be certified"
            )
        return self
...
class RecordRevision(BaseModel):
    ...
    ordinal: StrictInt
    supersedes_ref: TypedContentRef | None = None
    reason: StrictStr
...
class OfficialEvaluationRecord(BaseModel):
    ...
    aggregate_refs: tuple[TypedContentRef, ...]
...
class OfficialPlotManifest(BaseModel):
    ...
    record_refs: tuple[TypedContentRef, ...]
    # Complete aggregate + objective-selection references.
    aggregate_refs: tuple[TypedContentRef, ...]
    objective_selection_refs: tuple[TypedContentRef, ...]
