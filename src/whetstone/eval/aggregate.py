...
from whetstone.eval.attribution import require_exclusive_row_state
...
class EvalMatrixPlan:
    ...
    def __post_init__(self) -> None:
        expected_types = (
            ...
            ("seed_plan", self.seed_plan, SeedPlan),
            (
                "aggregation_config",
        ...
        if (
            sampling.get("seed_plan_hash")
            != self.seed_plan.identity_hash()
        ):
            raise ValueError(
                "sampling_config seed_plan_hash does not match seed_plan"
            )
        if self.task_set.selection_rule is not None:
            raise ValueError(
                "task_set must be an explicit task identity manifest"
            )
        if self.task_set.task_hashes != self.seed_plan.task_hashes:
            raise ValueError(
                "task_set and seed_plan task identities do not match"
            )
...
class TaskRows:
    ...
    def completed_rows(self, num_seeds: int) -> tuple[RowValue, ...]:

        if len(self.rows) > num_seeds:
            raise ValueError(
                f"task {self.task_hash} has {len(self.rows)} rows, "
                f"more than plan num_seeds {num_seeds}"
            )
        shortfall = num_seeds - len(self.rows)
...
class Aggregate:
    ...
    def __post_init__(self) -> None:
        ...
        planned = self.task_count * self.num_seeds
        counts = (
            self.rows_present,
            self.rows_missing,
            self.rows_failed,
...
def unweighted_task_mean(
    *,
    ...
    if not isinstance(plan, EvalMatrixPlan):
        raise TypeError("plan must be an EvalMatrixPlan")

    policy = plan.policy
    num_seeds = plan.seed_plan.num_seeds
    planned_task_hashes = plan.seed_plan.task_hashes
