...
class ToolCallStoreEntry(BaseModel):
    ...
    def _validate(self) -> ToolCallStoreEntry:
        ...
        if self.state is ToolCallState.COMPLETED:
            ...
            raise ValueError("only a completed entry references a Tool Result")
        return self
...
class ToolCallStore:
    ...
    def load_completed_result(self, entry: ToolCallStoreEntry) -> ToolResult:
        """Load and validate the persisted result for a completed entry."""
        if (
            entry.state is not ToolCallState.COMPLETED
            or entry.tool_result_ref is None
        ):
            raise ValueError("only a completed entry has a persisted result")
        if entry.tool_result_ref.schema_name != TOOL_RESULT_SCHEMA:
            raise ValueError("completed entry references a non-Tool Result")
        content = self._store.get(entry.tool_result_ref.reference)
        result = ToolResult.model_validate(content)
        if (
            result.tool_config_hash != entry.tool_config_hash
            or result.call_id != entry.call_id
            or tool_result_reference(result) != entry.tool_result_ref
        ):
            raise ValueError(
                "persisted Tool Result does not match its completed entry"
            )
        return result

    def persist_and_complete(self, result: ToolResult) -> ToolCallStoreEntry:
        """Persist a Tool Result before atomically recording completion."""
        expected = tool_result_reference(result)
        persisted, _status = self._store.put(
            TOOL_RESULT_SCHEMA, result.record_content()
        )
        if (
            persisted.schema != expected.schema_name
            or persisted.content_hash != expected.content_hash
        ):
            raise ValueError("persisted Tool Result failed content validation")
        return self.complete(result.tool_config_hash, result)
    ...
    def accept_or_refuse(
        ...
        with self._lock:
            ...
            decided = self._bind_entry(tool_config_hash, call.call_id, entry)
            if decided != entry:
